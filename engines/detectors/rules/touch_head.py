...
def _crown_and_radius(pose_lm):
    """Return (crown_x, crown_y, radius) from Pose landmarks, or fixed fallback."""
    if pose_lm is None:
    ...
    ear_shoulder_dist = shoulder_y - head_cy   # ears sit above shoulders → positive
    crown_y = head_cy - 0.4 * max(ear_shoulder_dist, 0.05)

    shoulder_width = abs(sh_l.x - sh_r.x)
    radius = max(0.15 * shoulder_width, 0.07)  # minimum 7% frame width

    return head_cx, crown_y, radius
...
def detect(landmarks, params: dict, context: dict) -> bool:
    ...
    crown_x, crown_y, radius = _crown_and_radius(pose_lm)

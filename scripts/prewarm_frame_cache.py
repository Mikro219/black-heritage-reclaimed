...
prewarm_frame_cache.py — Build every shot's framecache.npy pack ahead of time.
    ...
    py -3.12 scripts/prewarm_frame_cache.py --scenes export/scenes_generated --shots 02,03,04,09,10,12
    py -3.12 scripts/prewarm_frame_cache.py --scenes export/scenes_generated   (ALL shots — mind the disk!)
...
Resolution must match the runtime's display resolution (host profile
...
def runtime_resolution() -> tuple[int, int]:
    ...
    main.py's profile precedence (env/hostname -> profile display.resolution)
    and RenderEngine._get_resolution's fallback to config "resolution"."""
    try:
    ...
    name = os.environ.get("BHR_HOST_PROFILE")
    if not name:
        ...
        for path in PROFILES_DIR.glob("*.json"):
            if path.stem.lower() in hostname or hostname in path.stem.lower():
                ...
                break
    profile = {}
    for candidate in (name, "auto"):
        if candidate and (PROFILES_DIR / f"{candidate}.json").exists():
            with open(PROFILES_DIR / f"{candidate}.json", encoding="utf-8") as f:
                profile = json.load(f)
            break

    res = (profile.get("display", {}).get("resolution")
...
def main() -> int:
    ...
    parser.add_argument("--scenes", type=Path, default=ROOT / "scenes",
                        help="scenes root to prewarm (default the live scenes/)")
    ...
    frame_dirs = sorted(d for d in scenes.glob("act_*/shot_*/frames")
                        if d.is_dir())
    if args.shots:
        wanted = {s.strip().zfill(2) for s in args.shots.split(",") if s.strip()}
        frame_dirs = [d for d in frame_dirs
                      if d.parent.name.removeprefix("shot_") in wanted]
    if not frame_dirs:
        sys.exit(f"[prewarm] no matching act_*/shot_*/frames dirs under {scenes}")
